...
const UploadDetailsPage = () => {
  ...
  const handleDelete = async (index) => {
    ...
    const token = localStorage.getItem('token');
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/uploads/${file._id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    });
    if (res.status === 200) {
  ...
  // Placeholder for storage usage (to be implemented with backend later)
  const usedMB = uploadedFiles.reduce((acc, file) => acc + file.size, 0);
  const totalMB = 10;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-white px-6 py-10">
      ...
      <div className="mb-4 text-sm text-gray-600 dark:text-gray-400">
        <p>Total Files: <strong>{uploadedFiles.length}</strong> | Total Size: <strong>{usedMB.toFixed(2)} MB</strong></p>
            ...
            return (
              <li key={idx} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                      ...
                      • Uploaded: {new Date(file.date).toLocaleDateString()} • OTP:{' '}
                ...
                <div className="flex gap-2">
                  ...
                  <a
                    href={file.preview || '#'} // Fallback if preview is missing
                    download={file.name}
